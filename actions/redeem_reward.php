...
try {
    // 2. Obtener datos de la recompensa (Costo y Stock)
    $stmt_rew = $conn->prepare("SELECT cost_points, stock, active, name FROM rewards WHERE id = ? FOR UPDATE");
    ...
    $stmt_pts = $conn->prepare("SELECT available_points FROM user_stats WHERE user_id = ? FOR UPDATE");
    $stmt_pts->bind_param("i", $user_id);
    $stmt_pts->execute();
    $user_stats = $stmt_pts->get_result()->fetch_assoc();

    if (!$user_stats || $user_stats['available_points'] < $reward['cost_points']) {
    ...
    $stmt_update_pts = $conn->prepare("UPDATE user_stats SET available_points = available_points - ? WHERE user_id = ?");
    $stmt_update_pts->bind_param("ii", $costo, $user_id);
    $stmt_update_pts->execute();

    // 5. Restar stock a la recompensa
    $stmt_update_stock = $conn->prepare("UPDATE rewards SET stock = stock - 1 WHERE id = ?");
    $stmt_update_stock->bind_param("i", $reward_id);
    $stmt_update_stock->execute();

    // 6. Registrar el canje en el historial
    $stmt_log = $conn->prepare("INSERT INTO user_rewards (user_id, reward_id, points_spent, status) VALUES (?, ?, ?, 'pendiente')");
    ...
    header("Location: ../dashboard.php?view=store&error=" . $error_msg);
}