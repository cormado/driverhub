...
<style>
    ...
    .ticket-system { font-family: 'Montserrat', sans-serif; color: white; width: 100%; }
    .sys-modal { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.9); z-index: 9999; justify-content: center; align-items: center; }
    .sys-modal-box { width: 90%; max-width: 500px; background: #050505; border: 1px solid #333; padding: 30px; position: relative; box-shadow: 0 0 50px rgba(0,0,0,0.8); }
    .modal-danger { border: 2px solid #ff0055; box-shadow: 0 0 30px rgba(255, 0, 85, 0.2); }
    .modal-danger h3 { color: #ff0055; }
    .ticket-row { display: flex; justify-content: space-between; align-items: center; background: rgba(255,255,255,0.03); padding: 20px; border: 1px solid rgba(255,255,255,0.05); margin-bottom: 15px; transition: 0.3s; }
    .ticket-row:hover { border-color: #9d4edd; background: rgba(157, 78, 221, 0.05); }
    .status-badge { padding: 5px 10px; font-size: 0.7rem; font-family: 'Kanit'; text-transform: uppercase; border: 1px solid; display: inline-flex; align-items: center; gap: 5px; }
    .st-open { color: #00ff9d; border-color: #00ff9d; }
    .st-closed { color: #ff0055; border-color: #ff0055; }
    .st-process { color: #ffd700; border-color: #ffd700; }
    .chat-box { height: 500px; background: rgba(0,0,0,0.5); border: 1px solid #333; display: flex; flex-direction: column; }
    .chat-msgs { flex: 1; overflow-y: auto; padding: 20px; display: flex; flex-direction: column; gap: 15px; }
    .msg { padding: 15px; border-radius: 5px; max-width: 80%; font-size: 0.9rem; line-height: 1.5; }
    .msg-me { align-self: flex-end; background: rgba(0,255,157,0.1); border: 1px solid #00ff9d; color: white; }
    .msg-other { align-self: flex-start; background: #111; border: 1px solid #444; color: #ccc; }
    .chat-input-area { padding: 20px; background: #111; border-top: 1px solid #333; display: flex; gap: 10px; }
    .chat-input { flex: 1; background: #000; border: 1px solid #444; color: white; padding: 10px; resize: none; height: 50px; }
    .btn-create { background: rgba(0,255,157,0.1); border: 1px solid #00ff9d; color: #00ff9d; padding: 10px 20px; cursor: pointer; font-family: 'Kanit'; transition: 0.3s; display: inline-flex; align-items: center; gap: 8px; }
    .btn-create:hover { background: #00ff9d; color: black; }
    .btn-action { cursor: pointer; padding: 8px 15px; border: none; font-family: 'Kanit'; color: white; display: inline-flex; align-items: center; gap: 5px; }
    .btn-cancel { background: #333; color: white; padding: 10px 20px; border: none; cursor: pointer; font-family: 'Kanit'; transition: 0.2s; }
    .btn-cancel:hover { background: #555; }
    .btn-danger-confirm { background: #ff0055; color: white; padding: 10px 20px; border: none; cursor: pointer; font-family: 'Kanit'; transition: 0.2s; }
    .btn-danger-confirm:hover { background: #ff4477; box-shadow: 0 0 15px #ff0055; }
    .loader { width: 30px; height: 30px; border: 3px solid #333; border-top: 3px solid #00ff9d; border-radius: 50%; animation: spin 1s infinite linear; margin: 0 auto; }
                    ...
                    <div class="ticket-row">
                        ...
                        <div>
                            <?php 
                                ...
                                if($raw_st == 'abierto') { $st_txt = $tt['status_open']; $cls = 'st-open'; $icon_st = 'fa-envelope-open'; }
                                elseif($raw_st == 'resuelto') { $st_txt = $tt['status_closed']; $cls = 'st-closed'; $icon_st = 'fa-check-circle'; }
                                else { $st_txt = $tt['status_process']; $cls = 'st-process'; $icon_st = 'fa-spinner'; }
...
<script>
    ...
    if(formNuevo) {
        formNuevo.addEventListener('submit', function(e){
            ...
            fetch('includes/api_tickets.php', { method: 'POST', body: new FormData(this) })
            .then(res => res.text())
            .then(id => {
    ...
    function accionTicket(id, accion) {
        ...
        fetch('includes/api_tickets.php', { method: 'POST', body: data })
        .then(() => location.reload());
    }
